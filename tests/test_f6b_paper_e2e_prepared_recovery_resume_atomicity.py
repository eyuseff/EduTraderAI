...
def test_prepared_recovery_resume_rolls_back_staged_transition_on_crash(tmp_path) -> None:
    ...
    configuration = PaperExecutionPersistenceRuntimeConfiguration(
        ...
        application_version="f6b-prepared-recovery-resume-atomicity",
    ...
    try:
        ...
        recovery_transition = ExecutionTransitionRecord(
            transition_record_id="f6b-paper-e2e-prepared-runtime-crash-recovery",
            aggregate_id=local.aggregate_id,
        ...
        transitions_before = runtime._connection.execute(
            "SELECT count(*) FROM execution_transitions WHERE command_id=?",
            (str(recovery.command_id),),
        ).fetchone()[0]
        assert transitions_before == 0

        with pytest.raises(RuntimeError, match="synthetic crash before aggregate CAS"):
            with runtime.unit_of_work() as unit:
                assert (
                ...
                raise RuntimeError("synthetic crash before aggregate CAS")
    finally:
        runtime.close()
