...
def test_prepared_recovery_rolls_back_after_aggregate_cas_before_commit(tmp_path) -> None:
    ...
    configuration = PaperExecutionPersistenceRuntimeConfiguration(
        ...
        application_version="f6b-prepared-recovery-post-cas-atomicity",
    ...
    try:
        ...
        recovery_transition = ExecutionTransitionRecord(
            transition_record_id="f6b-paper-e2e-prepared-runtime-post-cas-crash-recovery",
            aggregate_id=local.aggregate_id,
        ...
        with pytest.raises(RuntimeError, match="synthetic crash after aggregate CAS"):
            with runtime.unit_of_work() as unit:
                ...
                staged = unit.aggregates.load_record(request.aggregate.aggregate_id)
                assert staged is not None
                assert staged.lifecycle_state is PaperExecutionLifecycleState.FILLED
                assert staged.execution_revision == PaperExecutionRevision(9)
                raise RuntimeError("synthetic crash after aggregate CAS")
    finally:
        runtime.close()
    ...
    try:
        ...
        assert durable.lifecycle_state is PaperExecutionLifecycleState.RECONCILIATION_REQUIRED
        ...
        discovered = restarted.discover_restart_candidates(
            ExecutionRestartDiscoveryQuery(
                lifecycle_states=(PaperExecutionLifecycleState.RECONCILIATION_REQUIRED,),
                schema_version=SCHEMA_VERSION,
            )
        )
