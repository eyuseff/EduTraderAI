...
def test_prepared_recovery_rolls_back_transition_and_aggregate_after_post_cas_crash(tmp_path) -> None:
    ...
    try:
        ...
        assert decision.next_state is PaperExecutionLifecycleState.FILLED
        assert decision.next_revision == PaperExecutionRevision(9)

        recovery_transition = ExecutionTransitionRecord(
            transition_record_id="f6b-paper-e2e-prepared-runtime-post-cas-crash",
            aggregate_id=local.aggregate_id,
