"""Tests for deterministic, fail-closed release evidence packaging."""
...
def test_build_evidence_pack_is_deterministic(tmp_path: Path) -> None:
    ...
    second = build / "second.zip"

    build_evidence_pack(tmp_path, manifest, first)
    build_evidence_pack(tmp_path, manifest, second)
...
def test_verify_evidence_pack_rejects_checksum_tampering(tmp_path: Path) -> None:
    ...
    with zipfile.ZipFile(output, "w") as archive:
        ...
        archive.writestr("evidence-manifest.json", json.dumps(resolved))
        archive.writestr(
            "SHA256SUMS",
...
def test_verify_evidence_pack_rejects_lost_human_review(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="lost human review"):
        verify_evidence_pack(output)
