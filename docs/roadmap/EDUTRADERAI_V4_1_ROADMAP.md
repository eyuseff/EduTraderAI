...
- ADR-006 — PAPER EXECUTION LIFECYCLE: PROPOSED.
...
- Next recommended implementation slice: V41-PQ-001F5D1 — EXECUTION LIFECYCLE CORE.
...
- V41-PQ-001F5D0 completes the Paper execution lifecycle design only. It proposes ADR-006, defines local lifecycle state versus broker truth separation, 30 proposed transitions, command/event separation, execution revision behavior, replay and duplicate semantics, unknown-outcome and reconciliation entry rules, cancellation and replacement semantics, partial-fill handling, dry-run isolation, and the recommendation to implement V41-PQ-001F5D1 lifecycle core before V41-PQ-001F5D2 deterministic dry-run executor. F5D0 adds no production lifecycle state machine, executor, dry-run executor, broker port, broker adapter, persistence, runtime wiring, event publisher, metrics, logging, configuration, dependency, or Live behavior.
