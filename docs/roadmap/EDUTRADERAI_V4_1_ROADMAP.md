...
- V41-PQ-001F5E2A completed the SQLite durable adapter design as documentation only. Sentinel ADR-008 review is completed and ADR-008 is Accepted. F5E2B is ready for implementation within a bounded schema, migration, connection bootstrap, PRAGMA verification, startup validation, quick-check/FK-check, and isolated temporary-database test scope only. The accepted design defines a local single-machine SQLite deployment envelope, local-filesystem rule, network-filesystem prohibition, single application authority, WAL/foreign-key/`synchronous = FULL`/busy-timeout requirements, `BEGIN IMMEDIATE` write transactions, exact aggregate CAS, append-only trigger protections, migration checksums, startup validation, backup/restore validation, corruption handling, and mandatory PostgreSQL migration triggers. No production SQLite adapter, database connection, repository implementation, runtime wiring, broker port, broker adapter, broker call, Paper trading enablement, or Live support was added.
